...
# Configuração CORS
CORS(
    app, resources={r"/api/*": {"origins": "*", "OrganizationId": ""}}
)  # Permitindo CORS apenas para rotas que começam com /api

api = Api(app)
route_register = RouteRegister(app)

# route_register.register_from_file(os.path.join(os.path.dirname(os.path.abspath(__file__)), "routes.yaml"))
route_register.register_routes_from_yaml(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "routes.yaml")
)
